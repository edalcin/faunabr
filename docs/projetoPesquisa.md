...
5. **Inteligência Artificial agêntica como instrumento e como objeto.** Todas as fases descritas acima — pesquisa bibliográfica, condução da entrevista de decisão, redação dos ADRs, construção do glossário, produção dos diagramas C4 e da documentação técnica — foram executadas em colaboração com um agente de IA (Anthropic Claude, operado no harness Oh My Pi). O mesmo vale para a fase seguinte: **o código das eventuais provas de conceito, MVPs e ferramentas derivadas desta arquitetura será gerado por IA agêntica**, a partir dos ADRs e do glossário deste repositório. Isso não é detalhe operacional: **é parte declarada do objeto de pesquisa**. Ver Seção 4.1.
...
- **Da decisão registrada ao código gerado.** Esta é a hipótese mais forte do projeto, e a que ainda não foi testada: se dezenove ADRs com alternativas rejeitadas, um glossário com termos a evitar e diagramas C4 em três níveis constituem **especificação suficiente** para que um agente gere código coerente com a arquitetura decidida — sem reintroduzir o que foi explicitamente rejeitado. O [ADR 0014](./adr/0014-licoes-do-pipeline-de-ingestao.md) existe em boa parte para esse fim: registrar por escrito o que **não** fazer, porque padrão herdado reaparece por inércia tanto em pessoa quanto em agente. Cada prova de conceito passa a ser, então, um teste da própria documentação: código que diverge da arquitetura indica ADR ambíguo, não apenas erro do agente.
